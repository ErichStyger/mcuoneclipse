...
#include "Midi.h"
#include "MidiMusic.h"
#if PL_HAS_MIDI_GET_READY
  #include "MidiMusicReady.h"
#endif
#if PL_HAS_MIDI_PIRATES_OF_CARIBIAN
  #include "MidiPirate.h"
#endif
#if PL_HAS_MIDI_HADDAWAY_WHAT_IS_LOVE
  #include "MidiHaddaway.h"
#endif
#if PL_HAS_MIDI_GAME_OF_THRONES
  #include "MidiGameOfThrones.h"
#endif
#if PL_HAS_MIDI_TETRIS
  #include "MidiTetris.h"
#endif
#if PL_HAS_MIDI_AXEL_F
  #include "MidiAxelF.h"
#endif
#if PL_HAS_MIDI_GHOSTBUSTERS
  #include "MidiGhostbusters.h"
#endif
#if PL_HAS_MIDI_JAMES_BOND
  #include "MidiBond.h"
#endif
#if PL_HAS_MIDI_STAR_WARS
...
void MM_NextSongNumber(void) {
  ...
  McuShell_SendStr((uint8_t*)"Next song: #", McuShell_GetStdio()->stdOut);
  McuShell_SendNum32u((uint32_t)MM_SetSong, McuShell_GetStdio()->stdOut);
...
void MM_PrevSongNumber(void) {
  ...
  McuShell_SendStr((uint8_t*)"Prev song: #", McuShell_GetStdio()->stdOut);
  McuShell_SendNum32u((uint32_t)MM_SetSong, McuShell_GetStdio()->stdOut);
  McuShell_SendStr((uint8_t*)"\r\n", McuShell_GetStdio()->stdOut);
...
static void Play(MIDI_SongNr song, MIDI_MusicTrack *tracks, unsigned int nofTracks, uint32_t tempoUS) {
  ...
  for(;;) { /* breaks, start playing song */
    McuShell_SendStr((uint8_t*)"Playing song: #", McuShell_GetStdio()->stdOut);
    McuShell_SendNum32u((uint32_t)song, McuShell_GetStdio()->stdOut);
    McuShell_SendStr((uint8_t*)"\r\n", McuShell_GetStdio()->stdOut);
    /* init defaults */
#if PL_HAS_FLOPPY
    FLOPPY_InitDrives();
    ...
    for(;;) { /* breaks, playing song */
      ...
      currTimeMs = (xTaskGetTickCount()-startTicks); // assuming 1 kHz tick count
      nofFinished = 0;
      for(channel=0;channel<nofTracks;channel++) {
    ...
    if (flags&MIDI_SONG_BIT_STOP) {
      McuShell_SendStr((uint8_t*)"Stopping song: #", McuShell_GetStdio()->stdOut);
      McuShell_SendNum32u((uint32_t)song, McuShell_GetStdio()->stdOut);
      McuShell_SendStr((uint8_t*)"\r\n", McuShell_GetStdio()->stdOut);
      for(channel=0;channel<nofTracks;channel++) {
#if PL_HAS_FLOPPY
    ...
    if (flags&MIDI_SONG_BIT_START) {
      McuShell_SendStr((uint8_t*)"Restart song: #", McuShell_GetStdio()->stdOut);
      McuShell_SendNum32u((uint32_t)song, McuShell_GetStdio()->stdOut);
      McuShell_SendStr((uint8_t*)"\r\n", McuShell_GetStdio()->stdOut);
      for(channel=0;channel<nofTracks;channel++) {
#if PL_HAS_FLOPPY
    ...
    if (flags&MIDI_SONG_BIT_NEXT) {
      McuShell_SendStr((uint8_t*)"Skipping to next song: #", McuShell_GetStdio()->stdOut);
      McuShell_SendNum32u((uint32_t)MM_SetSong, McuShell_GetStdio()->stdOut);
      McuShell_SendStr((uint8_t*)"\r\n", McuShell_GetStdio()->stdOut);
      for(channel=0;channel<nofTracks;channel++) {
#if PL_HAS_FLOPPY
...
void MM_Play(MIDI_SongNr song) {
  ...
  if (false) {
#if PL_HAS_MIDI_GET_READY
  } else if (song==MIDI_SONG_NR_GET_READY) {
...
#endif
#if PL_HAS_MIDI_STAR_WARS
  } else if (song==MIDI_SONG_NR_STAR_WARS) {
    nofTracks = StarWars_NofTracks();
    tempoUS = StarWars_GetTempoUS();
    offset = StarWars_GetOffset();
#endif
  } else {
    return; /* error */
  }
  ...
  if (false) {
    ...
    res = StarWars_GetMidiMusicInfo(&tracks[0], nofTracks);
#endif
  }
  if (res!=ERR_OK) {
    return; /* error case */
...
static void MidiPlayTask(void *pvParameters) {
  ...
  for(;;) {
    ...
    if (res==pdPASS && (flags&MIDI_SONG_BIT_START)) {
      ...
      if (false) {
#if PL_HAS_MIDI_GET_READY
      } else if (flags&MIDI_SONG_BIT_GET_READY) {
...
#endif
#if PL_HAS_MIDI_STAR_WARS
      } else if (flags&MIDI_SONG_BIT_STAR_WARS) {
        MM_Play(MIDI_SONG_NR_STAR_WARS);
#endif
      }
    }
  }
...
static uint8_t PrintStatus(McuShell_ConstStdIOType *io) {
  uint8_t buf[16];
  McuShell_SendStatusStr((unsigned char*)"midi", (const unsigned char*)"\r\n", io->stdOut);

  if (MM_IsPlaying()) {
    McuShell_SendStatusStr((unsigned char*)"  playing", (const unsigned char*)"yes\r\n", io->stdOut);
  } else {
    McuShell_SendStatusStr((unsigned char*)"  playing", (const unsigned char*)"no\r\n", io->stdOut);
  }
  McuUtility_Num32sToStr(buf, sizeof(buf), MM_SetSong);
  McuUtility_strcat(buf, sizeof(buf), (const unsigned char*)"\r\n");
  McuShell_SendStatusStr((unsigned char*)"  set song", buf, io->stdOut);
#if PL_HAS_FLOPPY
  McuShell_SendStatusStr((unsigned char*)"  hardware", (const unsigned char*)"Floppy\r\n", io->stdOut);
#else
  McuShell_SendStatusStr((unsigned char*)"  hardware", (const unsigned char*)"Buzzer\r\n", io->stdOut);
  McuUtility_Num8uToStr(buf, sizeof(buf), MM_Channel);
  McuUtility_strcat(buf, sizeof(buf), (const unsigned char*)"\r\n");
...
uint8_t MM_ParseCommand(const unsigned char *cmd, bool *handled, const McuShell_StdIOType *io) {
  const uint8_t *p;
  uint8_t res;
  int32_t tmp;

  if (McuUtility_strcmp((char*)cmd, McuShell_CMD_HELP)==0 || McuUtility_strcmp((char*)cmd, "midi help")==0) {
     McuShell_SendHelpStr((unsigned char*)"midi", (const unsigned char*)"Group of midi commands\r\n", io->stdOut);
     McuShell_SendHelpStr((unsigned char*)"  help|status", (unsigned char*)"Print help or status information\r\n", io->stdOut);
     McuShell_SendHelpStr((unsigned char*)"  (play|set) <number>", (const unsigned char*)"Play midi song\r\n", io->stdOut);
#if !PL_HAS_FLOPPY
     McuShell_SendHelpStr((unsigned char*)"  channel <number>", (const unsigned char*)"Midi channel for buzzer\r\n", io->stdOut);
#endif
#if PL_HAS_MIDI_GET_READY
     McuShell_SendHelpStr((unsigned char*)"      0", (const unsigned char*)"Get ready\r\n", io->stdOut);
#endif
#if PL_HAS_MIDI_PIRATES_OF_CARIBIAN
     McuShell_SendHelpStr((unsigned char*)"      1", (const unsigned char*)"Pirates of the Caribian\r\n", io->stdOut);
#endif
#if PL_HAS_MIDI_HADDAWAY_WHAT_IS_LOVE
     McuShell_SendHelpStr((unsigned char*)"      2", (const unsigned char*)"What is Love\r\n", io->stdOut);
#endif
#if PL_HAS_MIDI_GAME_OF_THRONES
     McuShell_SendHelpStr((unsigned char*)"      3", (const unsigned char*)"Game of Thrones\r\n", io->stdOut);
#endif
#if PL_HAS_MIDI_TETRIS
     McuShell_SendHelpStr((unsigned char*)"      4", (const unsigned char*)"Tetris\r\n", io->stdOut);
#endif
#if PL_HAS_MIDI_AXEL_F
     McuShell_SendHelpStr((unsigned char*)"      5", (const unsigned char*)"Axel F.\r\n", io->stdOut);
#endif
#if PL_HAS_MIDI_GHOSTBUSTERS
     McuShell_SendHelpStr((unsigned char*)"      6", (const unsigned char*)"Ghostbusters\r\n", io->stdOut);
#endif
#if PL_HAS_MIDI_JAMES_BOND
     McuShell_SendHelpStr((unsigned char*)"      7", (const unsigned char*)"James Bond\r\n", io->stdOut);
#endif
#if PL_HAS_MIDI_STAR_WARS
     McuShell_SendHelpStr((unsigned char*)"      8", (const unsigned char*)"Star Wars\r\n", io->stdOut);
#endif
     McuShell_SendHelpStr((unsigned char*)"  stop", (const unsigned char*)"Stop playing\r\n", io->stdOut);
     *handled = TRUE;
     return ERR_OK;
   } else if ((McuUtility_strcmp((char*)cmd, McuShell_CMD_STATUS)==0) || (McuUtility_strcmp((char*)cmd, "midi status")==0)) {
     *handled = TRUE;
     return PrintStatus(io);
   } else if (McuUtility_strcmp((char*)cmd, "midi stop")==0) {
     *handled = TRUE;
     MM_StopPlaying();
     return ERR_OK;
   } else if (McuUtility_strncmp((char*)cmd, "midi play", sizeof("midi play")-1)==0) {
     ...
     return res;
   } else if (McuUtility_strncmp((char*)cmd, "midi set", sizeof("midi set")-1)==0) {
     ...
     return res;
#if !PL_HAS_FLOPPY
   } else if (McuUtility_strncmp((char*)cmd, "midi channel", sizeof("midi channel")-1)==0) {
