...
uint8_t SHT31_WriteCommand(uint16_t cmd) {
  ...
  buf[0] = cmd>>8;
  buf[1] = cmd&0xff;
  return McuGenericI2C_WriteAddress(SHT31_I2C_ADDR, buf, sizeof(buf), NULL, 0);
}
...
uint8_t SHT31_ReadStatus(uint16_t *status) {
  ...
  res = McuGenericI2C_ReadAddress(SHT31_I2C_ADDR, cmd, sizeof(cmd), stat, sizeof(stat));
  if (res!=ERR_OK) {
    *status = 0;
    return res;
  }
...
uint8_t SHT31_ReadTempHum(float *temperature, float *humidity) {
  ...
  res = McuGenericI2C_ReadAddressWait(SHT31_I2C_ADDR, cmd, sizeof(cmd), 20, readbuffer, sizeof(readbuffer));
  if (res!=ERR_OK) {
    return res;
  }
