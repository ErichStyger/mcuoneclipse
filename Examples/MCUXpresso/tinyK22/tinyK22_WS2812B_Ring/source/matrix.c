...
#if PL_CONFIG_USE_STEPPER_EMUL
static bool MATRIX_LedFollowHands = true; /* if LEDs follow the hands */
#endif
...
bool MATRIX_GetLedFollowHands(void) {
  return MATRIX_LedFollowHands;
}

void MATRIX_SetLedFollowHands(bool enable) {
  MATRIX_LedFollowHands = enable;
}
#endif
...
#if PL_CONFIG_USE_STEPPER_EMUL
  if (MATRIX_LedFollowHands) {
    McuUtility_strcpy(buf, sizeof(buf), (unsigned char*)"LED follow hands: on");
  } else {
    McuUtility_strcpy(buf, sizeof(buf), (unsigned char*)"LED follow hands: off");
  }
  McuUtility_strcat(buf, sizeof(buf), (unsigned char*)"\r\n");
...
uint8_t MATRIX_ParseCommand(const unsigned char *cmd, bool *handled, const McuShell_StdIOType *io) {
            ...
            )
  {
...
#endif
#if PL_CONFIG_USE_STEPPER_EMUL
  } else if (McuUtility_strncmp((char*)cmd, "matrix color hand all ", sizeof("matrix color hand all ")-1)==0) {
    *handled = TRUE;
    p = cmd + sizeof("matrix color hand all ")-1;
    if (   McuUtility_xatoi(&p, &r)==ERR_OK && r>=0 && r<=0xff
  ...
  } else if (McuUtility_strncmp((char*)cmd, "matrix color hand ", sizeof("matrix color hand ")-1)==0) {
    *handled = TRUE;
    p = cmd + sizeof("matrix color hand ")-1;
    if (   McuUtility_xatoi(&p, &x)==ERR_OK && x>=0 && x<MATRIX_NOF_CLOCKS_X
        && McuUtility_xatoi(&p, &y)==ERR_OK && y>=0 && y<MATRIX_NOF_CLOCKS_Y
        && McuUtility_xatoi(&p, &z)==ERR_OK && z>=0 && z<MATRIX_NOF_CLOCKS_Z
    ...
    *handled = TRUE;
    if (MATRIX_GetLedFollowHands()) {
      McuShell_SendStr((unsigned char*)"matrix led follow hands is enabled, disable it first.\n", io->stdErr);
      return ERR_FAILED;
    }
  ...
  } else if (McuUtility_strcmp((char*)cmd, "matrix led follow hands on")==0) {
    *handled = TRUE;
    MATRIX_LedFollowHands = true;
    return ERR_OK;
  } else if (McuUtility_strcmp((char*)cmd, "matrix led follow hands off")==0) {
    *handled = TRUE;
    MATRIX_LedFollowHands = false;
    return ERR_OK;
#endif
