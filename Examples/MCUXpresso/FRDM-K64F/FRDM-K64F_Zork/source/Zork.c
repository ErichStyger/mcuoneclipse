...
static void ZorkTask(void *pvParameters) {
  ...
  if (run_zork_game()!=0) { /* run the game, does not return while it is running */
	CLS1_SendStr("Failed initializing game!\r\n", CLS1_GetStdio()->stdErr);
	vTaskResume(ShellTaskHandle); /* resume shell */
  }
  CLS1_SendStr("I know now why you cry. But it's something I can never do. Goodbye.\r\n", CLS1_GetStdio()->stdErr);
  vTaskDelay(pdMS_TO_TICKS(500)); /* give back some time to other tasks */
  vTaskDelete(NULL);  /* Hasta la vista, baby!  */
}

static uint8_t PrintHelp(const CLS1_StdIOType *io) {
  CLS1_SendHelpStr("Zork", "Zork commands\r\n", io->stdOut);
  CLS1_SendHelpStr("  help", "Print help\r\n", io->stdOut);
  CLS1_SendHelpStr("  play", "Play Zork game\r\n", io->stdOut);
  return ERR_OK;
}
