/* ###################################################################
**     This component module is generated by Processor Expert. Do not modify it.
**     Filename    : MINI1.h
**     Project     : FRDM-K64F_Zork
**     Processor   : MK64FN1M0VLQ12
**     Component   : minIni
**     Version     : Component 01.041, Driver 01.00, CPU db: 3.00.000
**     Compiler    : GNU C Compiler
**     Date/Time   : 2018-12-02, 21:22, # CodeGen: 12
**     Abstract    :
**         minIni is a programmer’s library to read and write ini files in embedded systems.
**     Settings    :
**          Component name                                 : MINI1
**          minIni Version                                 : 1.2B
**          Portable strnicmp()                            : yes
**          Use Real                                       : yes
**          Read Only                                      : no
**          No Debug                                       : yes
**          Global Buffer                                  : yes
**          FatFS                                          : 
**            FatFS                                        : FAT1
**            Buffer Size                                  : 64
**     Contents    :
**         ini_getbool    - int MINI1_ini_getbool(const mTCHAR *Section, const mTCHAR *Key, int DefValue,...
**         ini_gets       - int MINI1_ini_gets(const mTCHAR *Section, const mTCHAR *Key, const mTCHAR...
**         ini_puts       - int MINI1_ini_puts(const mTCHAR *Section, const mTCHAR *Key, const mTCHAR...
**         ini_getl       - long MINI1_ini_getl(const mTCHAR *Section, const mTCHAR *Key, long DefValue,...
**         ini_putl       - int MINI1_ini_putl(const mTCHAR *Section, const mTCHAR *Key, long Value,...
**         ini_getf       - INI_REAL MINI1_ini_getf(const mTCHAR *Section, const mTCHAR *Key, INI_REAL...
**         ini_putf       - int MINI1_ini_putf(const mTCHAR *Section, const mTCHAR *Key, INI_REAL Value,...
**         ini_getsection - int MINI1_ini_getsection(int idx, mTCHAR *Buffer, int BufferSize, const...
**         ini_getkey     - int MINI1_ini_getkey(const mTCHAR *Section, int idx, mTCHAR *Buffer, int...
**         ini_browse     - int MINI1_ini_browse(INI_CALLBACK Callback, const void *UserData, const TCHAR...
**
** (c) Copyright 2008-2012, CompuPhase;
** http      : www.compuphase.com
** Processor Expert port: Erich Styger, 2014-2017, http://www.mcuoneclipse.com
** License: See miniIni_LICENSE.txt and minIni_NOTICE.txt
** ###################################################################*/
/*!
** @file MINI1.h
** @version 01.00
** @brief
**         minIni is a programmer’s library to read and write ini files in embedded systems.
*/         
/*!
**  @addtogroup MINI1_module MINI1 module documentation
**  @{
*/         

#ifndef __MINI1_H
#define __MINI1_H

/* MODULE MINI1. */

/* Include shared modules, which are used for whole project */
#include "PE_Types.h"
#include "PE_Error.h"
#include "PE_Const.h"
#include "IO_Map.h"
/* Include inherited beans */
#include "FAT1.h"
#include "minIni.h" /* minIni Header file */

#include "Cpu.h"




#define MINI1_ini_getkey(Section, idx, Buffer, BufferSize, Filename) \
  ini_getkey(Section, idx, Buffer, BufferSize, Filename)
/*
** ===================================================================
**     Method      :  ini_getkey (component minIni)
**
**     Description :
**         Return the key inside a section
**     Parameters  :
**         NAME            - DESCRIPTION
**       * Section         - The name of the section to browse
**                           through, or NULL to browse through the keys
**                           outside any section
**         idx             - The zero-based sequence number of the key
**                           to return
**       * Buffer          - A pointer to the buffer to copy into
**         BufferSize      - The maximum number of
**                           characters to copy
**       * Filename        - The name and full path of the .
**                           ini file to read from
**     Returns     :
**         ---             - The number of characters copied into the
**                           supplied buffer
** ===================================================================
*/

#define MINI1_ini_getbool(Section, Key, DefValue, Filename) \
  ini_getbool(Section, Key, DefValue, Filename)
/*
** ===================================================================
**     Method      :  ini_getbool (component minIni)
**
**     Description :
**         Return a boolean key.  A true boolean is found if one of the
**         following is matched:
**         - A string starting with 'y' or 'Y'
**         - A string starting with 't' or 'T'
**         - A string starting with '1'
**         A false boolean is found if one of the following is matched:
**         - A string starting with 'n' or 'N'
**         - A string starting with 'f' or 'F'
**         - A string starting with '0'
**     Parameters  :
**         NAME            - DESCRIPTION
**       * Section         - The name of the section to search
**                           for
**       * Key             - The name of the entry to find the value of
**         DefValue        - Default value in the event of a
**                           failed read; it should be zero (0) or one
**                           (1).
**       * Filename        - The name and full path of the .
**                           ini file to read from
**     Returns     :
**         ---             - The true/false flag as interpreted at Key
** ===================================================================
*/

#define MINI1_ini_gets(Section, Key, DefValue, Buffer, BufferSize, Filename) \
  ini_gets(Section, Key, DefValue, Buffer, BufferSize, Filename)
/*
** ===================================================================
**     Method      :  ini_gets (component minIni)
**
**     Description :
**         Return the string for a given section and key.
**     Parameters  :
**         NAME            - DESCRIPTION
**       * Section         - The name of the section to search
**                           for
**       * Key             - Pointer to the name of the entry to find
**                           the value of
**       * DefValue        - Pointer to the default value in
**                           the event of a failed read
**       * Buffer          - A pointer to the buffer to copy into
**         BufferSize      - The maximum number of
**                           characters to copy
**       * Filename        - The name and full path of the .
**                           ini file to read from
**     Returns     :
**         ---             - The number of characters copied into the
**                           supplied buffer
** ===================================================================
*/

#define MINI1_ini_getl(Section, Key, DefValue, Filename) \
  ini_getl(Section, Key, DefValue, Filename)
/*
** ===================================================================
**     Method      :  ini_getl (component minIni)
**
**     Description :
**         Return the long integral value for a given section and key.
**     Parameters  :
**         NAME            - DESCRIPTION
**       * Section         - The name of the section to search
**                           for
**       * Key             - Pointer to the name of the entry to find
**                           the value of
**         DefValue        - Default value in the event of a
**                           failed read
**       * Filename        - The name and full path of the .
**                           ini file to read from
**     Returns     :
**         ---             - The value located at Key
** ===================================================================
*/

#define MINI1_ini_getsection(idx, Buffer, BufferSize, Filename) \
  ini_getsection(idx, Buffer, BufferSize, Filename)
/*
** ===================================================================
**     Method      :  ini_getsection (component minIni)
**
**     Description :
**         Return the key inside a section
**     Parameters  :
**         NAME            - DESCRIPTION
**         idx             - The zero-based sequence number of the
**                           section to return
**       * Buffer          - A pointer to the buffer to copy into
**         BufferSize      - The maximum number of
**                           characters to copy
**       * Filename        - The name and full path of the .
**                           ini file to read from
**     Returns     :
**         ---             - The number of characters copied into the
**                           supplied buffer
** ===================================================================
*/

#define MINI1_ini_getf(Section, Key, DefValue, Filename) \
  ini_getf(Section, Key, DefValue, Filename)
/*
** ===================================================================
**     Method      :  ini_getf (component minIni)
**
**     Description :
**         Return the long integral value for a given section and key.
**         Only enabled if 'Use Real' is enabled in properties.
**     Parameters  :
**         NAME            - DESCRIPTION
**       * Section         - The name of the section to search
**                           for
**       * Key             - Pointer to the name of the entry to find
**                           the value of
**         DefValue        - Default value in the event of a
**                           failed read
**       * Filename        - The name and full path of the .
**                           ini file to read from
**     Returns     :
**         ---             - The value located at Key
** ===================================================================
*/

#define MINI1_ini_putl(Section, Key, Value, Filename) \
  ini_putl(Section, Key, Value, Filename)
/*
** ===================================================================
**     Method      :  ini_putl (component minIni)
**
**     Description :
**         Writes a long integral value for a given section and key.
**         Method is only enabled if 'Read only' is set to  'no' in the
**         properties.
**     Parameters  :
**         NAME            - DESCRIPTION
**       * Section         - The name of the section to write
**                           the value in
**       * Key             - The name of the entry to write
**         Value           - The value to write
**       * Filename        - The name and full path of the .
**                           ini file to write to
**     Returns     :
**         ---             - 1 if successful, otherwise 0
** ===================================================================
*/

#define MINI1_ini_puts(Section, Key, Value, Filename) \
   ini_puts(Section, Key, Value, Filename);
/*
** ===================================================================
**     Method      :  ini_puts (component minIni)
**
**     Description :
**         Writes a string value for a given section and key. Method is
**         only enabled if 'Read only' is set to  'no' in the
**         properties.
**     Parameters  :
**         NAME            - DESCRIPTION
**       * Section         - The name of the section to write
**                           the string in
**       * Key             - The name of the entry to write
**       * Value           - Pointer to the buffer the string, or
**                           NULL to erase the key
**       * Filename        - The name and full path of the .
**                           ini file to write to
**     Returns     :
**         ---             - 1 if successful, otherwise 0
** ===================================================================
*/

#define MINI1_ini_putf(Section, Key, Value, Filename) \
  ini_putf(Section, Key, Value, Filename)
/*
** ===================================================================
**     Method      :  ini_putf (component minIni)
**
**     Description :
**         Writes a long float value for a given section and key.
**         Method is only enabled if 'Read only' is set to  'no' in the
**         properties and 'Use Real' is enabled.
**     Parameters  :
**         NAME            - DESCRIPTION
**       * Section         - The name of the section to write
**                           the value in
**       * Key             - The name of the entry to write
**         Value           - The value to write
**       * Filename        - The name and full path of the .
**                           ini file to write to
**     Returns     :
**         ---             - 1 if successful, otherwise 0
** ===================================================================
*/

#define MINI1_ini_browse(Callback, UserData, Filename) \
  ini_browse(Callback, UserData, Filename)
/*
** ===================================================================
**     Method      :  ini_browse (component minIni)
**
**     Description :
**         Function to browse the ini file. With this function the file
**         is opened only once.
**     Parameters  :
**         NAME            - DESCRIPTION
**         Callback        - a pointer to a function that
**                           will be called for every setting in the INI
**                           file.
**         UserData        - arbitrary data, which the
**                           function passes on the the Callback function
**         Filename        - the name and full path of the .
**                           ini file to read from
**     Returns     :
**         ---             - 1 on success, 0 on failure (INI file not
**                           found)
** ===================================================================
*/

/* END MINI1. */

#endif
/* ifndef __MINI1_H */
/*!
** @}
*/
