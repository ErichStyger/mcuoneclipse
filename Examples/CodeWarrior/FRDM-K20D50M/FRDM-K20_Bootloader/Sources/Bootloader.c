...
//#define APP_FLASH_VECTOR_SIZE       0xc0    /* size of vector table KL25 */
#define APP_FLASH_VECTOR_SIZE       0xF8    /* size of vector table K20 */
...
//  PORTC_PCR0 = (uint32_t)((PORTC_PCR0 & (uint32_t)~0x01000600UL) | (uint32_t)0x0001UL);
...
//  PORT_PDD_SetPinPullEnable(PORTB_BASE_PTR, 8, PORT_PDD_PULL_ENABLE);
  PORT_PDD_SetPinPullSelect(PORTC_BASE_PTR, 0, PORT_PDD_PULL_UP);
  PORT_PDD_SetPinPullEnable(PORTC_BASE_PTR, 0, PORT_PDD_PULL_ENABLE);
  //plante btl!!  WAIT1_Waitms(5); /* wait get pull-up a chance to pull-up */
  //return TRUE;// test
#endif
  pc0 = BitIoLdd1_GetVal((LDD_TDeviceData*)PTC_BASE_PTR);
  if (!pc0) { /* button pressed (has pull-up!) */
    //WAIT1_Waitms(50); /* wait to debounce */
	pc0 = BitIoLdd1_GetVal((LDD_TDeviceData*)PTC_BASE_PTR);
    if (!pc0) { /* still pressed */
...
//  if (!BL_CheckBootloaderMode()) { /*Sans le test VecteurReset app.... we do have a valid application vector? -1/0xfffffff would mean flash erased */
