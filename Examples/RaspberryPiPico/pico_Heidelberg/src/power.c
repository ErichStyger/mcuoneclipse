...
#if !PL_CONFIG_USE_PICO_W /* on Pico W, pin access is through the cyw43_arch */
    struct ps {
      McuGPIO_Handle_t pin; /* low active */
    } ps;
#endif
...
bool Power_GetPsIsOn(void) {
  ...
  return !cyw43_arch_gpio_get(POWER_CONFIG_PS_PIN); /* false/LOW: on, true/HIGH: off */
#else
...
static uint8_t PrintStatus(McuShell_ConstStdIOType *io) {
  ...
  #endif
  McuUtility_strcat(buf, sizeof(buf), Power_GetPsIsOn()?(unsigned char*)", power save on\r\n":(unsigned char*)", power save off\r\n");
...
void Power_Init(void) {
#if POWER_CONFIG_USE_EN_VCC2
  {
    ...
    config.isHighOnInit = false; /* VCC2 PMOSFET pin is low active: turn on power at startup */
