...
static err_t tcp_server_sent(void *arg, struct tcp_pcb *tpcb, u16_t len) {
  ...
  if (state->sent_len >= BUF_SIZE) {
    ...
    state->recv_len = 0;
    McuLog_trace("Waiting for buffer from client");
  }
  return ERR_OK;
}
...
static void tcpSrvTask(void *pv) {
  for(;;) {
    vTaskDelay();
  }
}
...
void TcpServer_Init(void) {
  if (xTaskCreate(
      tcpSrvTask,  /* pointer to the task */
      "tcpSrv", /* task name for kernel awareness debugging */
