...
typedef struct DBNC_FSMData {
  ...
  uint8_t longKeyCnt; /*!< counting how long we press a key */
  TRG_TriggerKind trigger; /*!< trigger to be used to iterate through state machine */
  uint8_t debounceTicks; /*!< number of trigger ticks needed for debouncing */
  uint8_t longKeyTicks; /*!< number of trigger ticks needed for long key press */
} DBNC_FSMData;
