...
static void PrintHelp(const FSSH1_StdIOType *io) {
  ...
  FSSH1_SendHelpStr((unsigned char*)"my help", (unsigned char*)"My help text\r\n", io->stdOut);
}
...
static uint8_t ParseCommand(const unsigned char *cmd, bool *handled, const FSSH1_StdIOType *io) {
  ...
  if (strcmp((char*)cmd, (char*)FSSH1_CMD_HELP)==0) {
    PrintHelp(io);
    *handled = TRUE;
  } else if (strcmp((char*)cmd, (char*)FSSH1_CMD_STATUS)==0) {
    PrintStatus(io);
    *handled = TRUE;
  }
