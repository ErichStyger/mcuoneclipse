 ...
 */
DioIrqHandler *DioIrq[] = { SX1272OnDio0Irq, SX1272OnDio1Irq, SX1272OnDio2Irq, SX1272OnDio3Irq,
...
void SX1272SetRxConfig( RadioModems_t modem, uint32_t bandwidth, uint32_t datarate,
    ...
    switch ( modem ) {
        case MODEM_FSK:
        {
            ...
            SX1272Write(REG_PACKETCONFIG1,
                    (SX1272Read(REG_PACKETCONFIG1) & RF_PACKETCONFIG1_CRC_MASK
        ...
        case MODEM_LORA:
        {
            ...
            SX1272Write(REG_LR_MODEMCONFIG1,
                    (SX1272Read(REG_LR_MODEMCONFIG1) & RFLR_MODEMCONFIG1_BW_MASK
            ...
            SX1272Write(REG_LR_MODEMCONFIG2,
                    (SX1272Read(REG_LR_MODEMCONFIG2) & RFLR_MODEMCONFIG2_SF_MASK
            ...
            if ( SX1272.Settings.LoRa.FreqHopOn == true ) {
                SX1272Write(REG_LR_PLLHOP,
                        (SX1272Read(REG_LR_PLLHOP) & RFLR_PLLHOP_FASTHOP_MASK)
            ...
            if ( datarate == 6 ) {
                SX1272Write(REG_LR_DETECTOPTIMIZE,
                        (SX1272Read(REG_LR_DETECTOPTIMIZE) & RFLR_DETECTIONOPTIMIZE_MASK)
                                | RFLR_DETECTIONOPTIMIZE_SF6);
                SX1272Write(REG_LR_DETECTIONTHRESHOLD, RFLR_DETECTIONTHRESH_SF6);
            } else {
                SX1272Write(REG_LR_DETECTOPTIMIZE,
                        (SX1272Read(REG_LR_DETECTOPTIMIZE) & RFLR_DETECTIONOPTIMIZE_MASK)
                                | RFLR_DETECTIONOPTIMIZE_SF7_TO_SF12);
                SX1272Write(REG_LR_DETECTIONTHRESHOLD, RFLR_DETECTIONTHRESH_SF7_TO_SF12);
            }
        }
            break;
    }
...
void SX1272SetTxConfig( RadioModems_t modem, int8_t power, uint32_t fdev, uint32_t bandwidth,
...
uint32_t SX1272GetTimeOnAir( RadioModems_t modem, uint8_t pktLen )
{
    ...
    switch ( modem ) {
        case MODEM_FSK:
        {
            airTime = round(
                    (8
                            * (SX1272.Settings.Fsk.PreambleLen
                                    + ((SX1272Read(REG_SYNCCONFIG) & ~RF_SYNCCONFIG_SYNCSIZE_MASK)
                                            + 1)
        ...
        case MODEM_LORA:
        {
            ...
            double tmp = ceil(
                    (8 * pktLen - 4 * SX1272.Settings.LoRa.Datarate + 28
                            + 16 * SX1272.Settings.LoRa.CrcOn
        ...
        case MODEM_LORA:
        {
            if ( SX1272.Settings.LoRa.IqInverted == true ) {
                SX1272Write(REG_LR_INVERTIQ,
                        ((SX1272Read(REG_LR_INVERTIQ) & RFLR_INVERTIQ_TX_MASK
...
void SX1272SetRx( uint32_t timeout )
{
    ...
    switch ( SX1272.Settings.Modem ) {
        case MODEM_FSK:
        {
            ...
            // DIO4=Preamble
            // DIO5=ModeReady
            SX1272Write(REG_DIOMAPPING1,
                    (SX1272Read(REG_DIOMAPPING1) & RF_DIOMAPPING1_DIO0_MASK
                            & RF_DIOMAPPING1_DIO2_MASK) | RF_DIOMAPPING1_DIO0_00
                            | RF_DIOMAPPING1_DIO2_11);

            SX1272Write(REG_DIOMAPPING2,
                    (SX1272Read(REG_DIOMAPPING2) & RF_DIOMAPPING2_DIO4_MASK
        ...
        case MODEM_LORA:
        {
            ...
            if ( SX1272.Settings.LoRa.FreqHopOn == true ) {
                ...
                SX1272Write(REG_DIOMAPPING1,
                        (SX1272Read(REG_DIOMAPPING1) & RFLR_DIOMAPPING1_DIO0_MASK
        ...
        if ( rxContinuous == false ) {
            TimerSetValue(&RxTimeoutSyncWord,
                    (8.0
                            * (SX1272.Settings.Fsk.PreambleLen
                                    + ((SX1272Read(REG_SYNCCONFIG) & ~RF_SYNCCONFIG_SYNCSIZE_MASK)
                                            + 1.0) + 10.0) / (double) SX1272.Settings.Fsk.Datarate)
                            * 1e6);
...
void SX1272SetModem( RadioModems_t modem )
{
    ...
    switch ( SX1272.Settings.Modem ) {
        ...
        case MODEM_FSK:
            SX1272SetOpMode (RF_OPMODE_SLEEP);
            SX1272Write(REG_OPMODE,
                    (SX1272Read(REG_OPMODE) & RFLR_OPMODE_LONGRANGEMODE_MASK)
        ...
        case RF_RX_RUNNING:
            if ( SX1272.Settings.Modem == MODEM_FSK ) {
                ...
                if ( SX1272.Settings.Fsk.RxContinuous == true ) {
                    ...
                    SX1272Write(REG_RXCONFIG,
                            SX1272Read(REG_RXCONFIG) | RF_RXCONFIG_RESTARTRXWITHOUTPLLLOCK);
                } else {
                    SX1272.Settings.State = RF_IDLE;
            ...
            switch ( SX1272.Settings.Modem ) {
                case MODEM_FSK:
                    ...
                    if ( (SX1272.Settings.FskPacketHandler.Size == 0)
                        ...
                        SX1272ReadFifo(RxBuffer + SX1272.Settings.FskPacketHandler.NbBytes,
                    ...
                    if ( (RadioEvents != NULL) && (RadioEvents->RxDone != NULL) ) {
                        RadioEvents->RxDone(RxBuffer, SX1272.Settings.FskPacketHandler.Size,
                ...
                case MODEM_LORA:
                {
                    ...
                    if ( snr < 0 ) {
                        SX1272.Settings.LoRaPacketHandler.RssiValue = RSSI_OFFSET + rssi
                                + (rssi >> 4) + snr;
                    } else {
                        SX1272.Settings.LoRaPacketHandler.RssiValue = RSSI_OFFSET + rssi
                                + (rssi >> 4);
                    ...
                    if ( (RadioEvents != NULL) && (RadioEvents->RxDone != NULL) ) {
                        RadioEvents->RxDone(RxBuffer, SX1272.Settings.LoRaPacketHandler.Size,
        ...
        case RF_RX_RUNNING:
            switch ( SX1272.Settings.Modem ) {
                case MODEM_FSK:
                    ...
                    if ( (SX1272.Settings.FskPacketHandler.Size
                            - SX1272.Settings.FskPacketHandler.NbBytes)
                            > SX1272.Settings.FskPacketHandler.FifoThresh ) {
                        SX1272ReadFifo((RxBuffer + SX1272.Settings.FskPacketHandler.NbBytes),
                                SX1272.Settings.FskPacketHandler.FifoThresh);
                        SX1272.Settings.FskPacketHandler.NbBytes +=
                                SX1272.Settings.FskPacketHandler.FifoThresh;
        ...
        case RF_TX_RUNNING:
            switch ( SX1272.Settings.Modem ) {
                case MODEM_FSK:
                    ...
                    if ( (SX1272.Settings.FskPacketHandler.Size
                            - SX1272.Settings.FskPacketHandler.NbBytes)
                            > SX1272.Settings.FskPacketHandler.ChunkSize ) {
                        SX1272WriteFifo((RxBuffer + SX1272.Settings.FskPacketHandler.NbBytes),
                                SX1272.Settings.FskPacketHandler.ChunkSize);
                        SX1272.Settings.FskPacketHandler.NbBytes +=
                                SX1272.Settings.FskPacketHandler.ChunkSize;
                    } else {
            ...
            switch ( SX1272.Settings.Modem ) {
                case MODEM_FSK:
                    if ( (SX1272.Settings.FskPacketHandler.PreambleDetected == true)
                        ...
                        SX1272.Settings.FskPacketHandler.RssiValue = -(SX1272Read(REG_RSSIVALUE)
                                >> 1);

                        SX1272.Settings.FskPacketHandler.AfcValue =
                                (int32_t) (double) (((uint16_t) SX1272Read(REG_AFCMSB) << 8)
                                        | (uint16_t) SX1272Read(REG_AFCLSB)) * (double) FREQ_STEP;
                    ...
                    if ( SX1272.Settings.LoRa.FreqHopOn == true ) {
                        ...
                        if ( (RadioEvents != NULL) && (RadioEvents->FhssChangeChannel != NULL) ) {
                            RadioEvents->FhssChangeChannel(
                                    (SX1272Read(REG_LR_HOPCHANNEL) & RFLR_HOPCHANNEL_CHANNEL_MASK));
                        }
                    }
                    break;
                default:
...
void SX1272OnDio3Irq( void )
{
    switch ( SX1272.Settings.Modem ) {
        case MODEM_FSK:
            break;
        case MODEM_LORA:
            if ( (SX1272Read(REG_LR_IRQFLAGS) & RFLR_IRQFLAGS_CADDETECTED)
                    == RFLR_IRQFLAGS_CADDETECTED ) {
                // Clear Irq
