/* ###################################################################
**     Filename    : main.c
**     Project     : FRDM-K64F_NVIC
**     Processor   : MK64FN1M0VLQ12
**     Version     : Driver 01.01
**     Compiler    : GNU C Compiler
**     Date/Time   : 2016-08-17, 18:27, # CodeGen: 0
**     Abstract    :
**         Main module.
...
static void SetGroupingPrio(void) {
  //#define configLIBRARY_MAX_SYSCALL_INTERRUPT_PRIORITY 2  // ==> BASEPRI 0x20  (main 2, sub 0)

  uint32_t grp;
  uint32_t prio;

  grp = NVIC_GetPriorityGrouping();
  ...
  prio = NVIC_GetPriority(-5); /* SVCall */
  prio = NVIC_GetPriority(-2); /* PendSV */
...
static void SetPrio(void) {
  ...
  a = NVIC_GetPriority(58-16); /* returns the 'non-shifted' value! */
  b = NVIC_GetPriority(59-16); /* returns the 'non-shifted' value! */
  NVIC_SetPriority(58-16, (1UL<<__NVIC_PRIO_BITS)-1UL);
  NVIC_SetPriority(59-16, (1UL<<__NVIC_PRIO_BITS)-1UL);

  NVIC_SetPriority(58-16, 0x7); /* IRQ Number 42, Interrupt Priority value 0x70 */
  NVIC_SetPriority(59-16, 0xA); /* IRQ Number 43, Interrupt Priority value 0xA0 */

 // NVIC_SetPriority(58-16, 0xA); /* IRQ Number 42, Interrupt Priority 0xA */
 // NVIC_SetPriority(59-16, 0x7); /* IRQ Number 43, Interrupt Priority 0x7 */
}
