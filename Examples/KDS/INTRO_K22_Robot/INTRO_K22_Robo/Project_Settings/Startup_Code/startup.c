...
static void __copy_rom_section(unsigned long dst, unsigned long src, unsigned long size)
{
	...
	while( len > 0)
	{
		...
		if( !(src & mask_int) && !(dst & mask_int) && len >= (unsigned long)size_int)
		{
			*((int *)dst)  = *((int*)src);
			dst += size_int;
			src += size_int;
		...
		else if( !(src & mask_short) && !(dst & mask_short) && len >= (unsigned long)size_short)
		{
			*((short *)dst)  = *((short*)src);
			dst += size_short;
			src += size_short;
...
static void zero_fill_bss(void)
{
  ...
  unsigned long dst = (unsigned long) __START_BSS;
	...
	while( len > 0)
	{
		...
		if( !(dst & mask_int) && len >= (unsigned long)size_int)
		{
			*((int *)dst)  = 0;
			dst += size_int;
			len -= size_int;
		}
		else if( !(dst & mask_short) && len >= (unsigned long)size_short)
		{
			*((short *)dst)  = 0;
			dst += size_short;
			len -= size_short;
