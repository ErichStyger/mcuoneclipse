...
#define FLOPPY_HIGHEST_NOTE  48 /* highest note/tone we can play */
static const uint16_t FLOPPY_NoteTicks[FLOPPY_NOF_NOTES] = {
    ...
    54, // 46
    51, // 47
    48, // 48 (that's the highest note we can play (FLOPPY_HIGHEST_NOTE)!)
    48, // 49
    48 , // Note 50
    48 , // Note 51
    48 , // Note 52
...
void FLOPPY_MIDI_NoteOn(uint8_t channel, uint8_t note, uint8_t velocity) {
  ...
  if (tmp<0) {
    tmp = 0;
  } else if (tmp>FLOPPY_HIGHEST_NOTE) {
    tmp = FLOPPY_HIGHEST_NOTE;
  }
  note = tmp;
