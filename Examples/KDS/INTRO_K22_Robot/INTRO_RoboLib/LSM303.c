...
void LSM_MagEnable(void) {
  ...
  (void)GI2C1_WriteAddress(I2C_MAG_ADDR, &addr, sizeof(addr), &data, sizeof(data));
}

void LSM_MagTempEnable(void) {
  /* enable magnetometer temperature sensor */
...
void LSM_MagReadTemperature(uint16_t *celsius) {
  ...
  (void)GI2C1_ReadAddress(I2C_MAG_ADDR, &addr, sizeof(addr), buf, sizeof(buf));
