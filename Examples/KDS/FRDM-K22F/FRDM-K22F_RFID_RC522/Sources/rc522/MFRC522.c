 ...
 * The interface is described in the datasheet section 8.1.2.
 */
void PCD_WriteRegisterBlock(	PCD_Register reg,	///< The register to write to. One of the PCD_Register enums.
 ...
 * The interface is described in the datasheet section 8.1.2.
 */
void PCD_ReadRegisterBlock(	PCD_Register reg,	///< The register to read from. One of the PCD_Register enums.
 ...
 */
MFRC522_StatusCode PCD_CalculateCRC(	uint8_t *data,		///< In: Pointer to the data to transfer to the FIFO for CRC calculation.
...
bool MFRC522_PCD_PerformSelfTest(void) {
	...
	PCD_WriteRegisterBlock(FIFODataReg, 25, ZEROES);	// write 25 bytes of 00h to FIFO
...
MFRC522_StatusCode PCD_CommunicateWithPICC(	uint8_t command,		///< The command to execute. One of the PCD_Command enums.
  ...
  uint8_t bitFraming = (rxAlign << 4) + txLastBits;		// RxAlign = BitFramingReg[6..4]. TxLastBits = BitFramingReg[2..0]
	...
	if (backData && backLen && checkCRC) {
		...
		MFRC522_StatusCode status = PCD_CalculateCRC(&backData[0], *backLen - 2, &controlBuffer[0]);
		if (status != STATUS_OK) {
			return status;
		}
 ...
 */
MFRC522_StatusCode MFRC522_PCD_Authenticate(byte command,		///< PICC_CMD_MF_AUTH_KEY_A or PICC_CMD_MF_AUTH_KEY_B
  ...
  uint8_t sendData[12];
	...
	return PCD_CommunicateWithPICC(PCD_MFAuthent, waitIRq, &sendData[0], sizeof(sendData), NULL, 0, NULL, 0, FALSE);
