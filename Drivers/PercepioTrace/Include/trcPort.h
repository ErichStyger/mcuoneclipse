...
#include "trcBase.h"
#if 1 /* << EST */
#include "portmacro.h" /* for FREERTOS_HWTC_DOWN_COUNTER */
#endif
...
#define PORT_FREESCALE_PROCESSOR_EXPERT       11 /* << EST */
 ...
 * - Freescale Kinetis: 0  (lower irq priority values are more significant)
 *
 ...
 ******************************************************************************/
  ...
  /* only define dummy values. The real values are in portTicks.h */
  #define HWTC_COUNT_DIRECTION
  #define HWTC_PERIOD
  #define HWTC_DIVISOR 1
  #define HWTC_COUNT
 ...
 ******************************************************************************/         
#if 1 /* << EST */
%if %TraceConsoleEnabled='yes'
#define vTraceConsoleMessage(x)  {extern void DBGU_Print_Ascii(char *buffer); DBGU_Print_Ascii(x);}
%else
#define vTraceConsoleMessage(x)  ((void)(x)) /* do nothing */
%endif

#define SPRINTF_BUF_SIZE %sprintfBufferSize /* size of global buffer */
extern char sprintBuffer[SPRINTF_BUF_SIZE];
#else
#define vTraceConsoleMessage(str) vConsolePrintMessage(str)
#endif
