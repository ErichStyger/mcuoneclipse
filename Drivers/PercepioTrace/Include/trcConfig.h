 ...
 * either case.
 *****************************************************************************/
/* floating point format is supported in vTracePrintF() */
 ...
 *****************************************************************************/
#if 1 /* << EST */
#define RECORDER_STORE_MODE %RecorderStoreMode
#else
#ifndef WIN32
 ...
 * Default is 0.
 *
 * If enabled (1), the recorder will count each type of event, in order to give
 * an immediate picture (in a console print), regarding what events that is
 * using significant buffer space.
 * In the Windows emulation port, vTracePortEnd() will print this info when
 * the recorder is stopped. This is not recommended in a real-time system, as
 * vTracePortEnd is called from the kernel and blocks it until vTracePortEnd
 * returns. This uses some RAM, so make sure to disable it if you don't use it.
 ******************************************************************************/
#if 1 /* << EST */
%if %IncludeEventStats='yes'
#define INCLUDE_EVENT_STATS 1
%else
#define INCLUDE_EVENT_STATS 0
%endif
#define EVENT_COUNT_SIZE %EventCountSize /* size for the eventCount[] array */
#define EVENT_SPRINT_F_BUF_SIZE  %sprintfBufferSize /* size for sprintfBuffer[] */
#else
#define INCLUDE_EVENT_STATS 0
#endif
 ...
 *****************************************************************************/
#if 1 /* << EST */
%if %UseTraceProgressMonitorTask='yes'
#define TRACE_PROGRESS_MONITOR_TASK_PRIORITY        %TraceProgressMonitorTaskPriorityString
#define TRACE_PROGRESS_MONITOR_TASK_LOCAL_BUF_SIZE  %vTraceMonitorTaskBufferSize  /* used for localsprintbuffer[] inside vTraceMonitorTask() */
#define TRACE_PROGRESS_TASK_NAME                    "%TaskNameString"
%else
#define TRACE_PROGRESS_MONITOR_TASK_PRIORITY        (tskIDLE_PRIORITY + 1)
#define TRACE_PROGRESS_MONITOR_TASK_LOCAL_BUF_SIZE  80  /* used for localsprintbuffer[] inside vTraceMonitorTask() */
#define TRACE_PROGRESS_TASK_NAME                    "TraceMon"
%endif
#else
#define TRACE_PROGRESS_MONITOR_TASK_PRIORITY tskIDLE_PRIORITY + 1
#endif
...
#if 1 /* << EST */
%if %UseTraceProgressMonitorTask='yes'
#define TRACE_PROGRESS_MONITOR_TASK_STACKSIZE       %TraceProgressMonitorTaskStackString
%else
...
#if 1 /* << EST */
%if %UseTraceProgressMonitorTask='yes'
    #define TRACE_PROGRESS_MONITOR_TASK_PERIOD %TraceProgressMonitorPeriod
%else
