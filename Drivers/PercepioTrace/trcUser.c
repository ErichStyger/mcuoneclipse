...
static void vTraceMonitorTask(void)
{
...
#if 1 /* << EST */
    char localsprintbuffer[TRACE_PROGRESS_MONITOR_TASK_LOCAL_BUF_SIZE];
#else
    ...
    for (;;)
    {
        ...
        {
#if 1 /* << EST */
            %@Utility@'ModuleName'%.strcpy((unsigned char*)localsprintbuffer, sizeof(localsprintbuffer), (unsigned char*)"\n\r[FreeRTOS+Trace] Error: ");
            %@Utility@'ModuleName'%.strcat((unsigned char*)localsprintbuffer, sizeof(localsprintbuffer), (unsigned char*)err);
            %@Utility@'ModuleName'%.strcat((unsigned char*)localsprintbuffer, sizeof(localsprintbuffer), (unsigned char*)"\n\r");
#else
        ...
        if (state == STATE_STARTED)
        {
            ...
            {
#if 1 /* << EST */
            %@Utility@'ModuleName'%.strcpy((unsigned char*)localsprintbuffer, sizeof(localsprintbuffer), (unsigned char*)"\n\r[FreeRTOS+Trace] Event count: ");
            %@Utility@'ModuleName'%.strcatNum32u((unsigned char*)localsprintbuffer, sizeof(localsprintbuffer), RecorderDataPtr->numEvents);
            %@Utility@'ModuleName'%.strcat((unsigned char*)localsprintbuffer, sizeof(localsprintbuffer), (unsigned char*)", Duration: ");
            %@Utility@'ModuleName'%.strcatNum32u((unsigned char*)localsprintbuffer, sizeof(localsprintbuffer), RecorderDataPtr->absTimeLastEventSecond*1000 + RecorderDataPtr->absTimeLastEvent / (RecorderDataPtr->frequency / 1000));
            %@Utility@'ModuleName'%.strcat((unsigned char*)localsprintbuffer, sizeof(localsprintbuffer), (unsigned char*)" ms.\n\r");
#else
                sprintf(localsprintbuffer,
                    "\n\r[FreeRTOS+Trace] Event count: %%d, Duration: %%d ms. [%%d]\n\r",
...
#if 1 /* << EST: use real time, not ticks, for better portability */
        vTaskDelayUntil( &xNextWakeTime, TRACE_PROGRESS_MONITOR_TASK_PERIOD/portTICK_RATE_MS);
#else
...
#if 1 /* << EST */
   (void)xTaskCreate((pdTASK_CODE)vTraceMonitorTask, (const signed char*)TRACE_PROGRESS_TASK_NAME, TRACE_PROGRESS_MONITOR_TASK_STACKSIZE, NULL, TRACE_PROGRESS_MONITOR_TASK_PRIORITY, NULL );
#else
